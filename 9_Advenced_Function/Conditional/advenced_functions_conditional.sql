create database advenced_functions_conditional;
use advenced_functions_conditional;
...
-- 1.CASE: Hiển thị trạng thái đơn hàng theo orders.status → "Đang chờ", "Đang giao", "Hoàn tất".
select o.id as order_id, o.status as order_status,
case
	when o.status = "Pending" then "Dang cho"
    when o.status = "Shipped" then "Dang giao"
    else "Hoan tat"
end as order_status
from orders o;

-- 2. CASE: Tạo cột mới trong products để phân loại giá (<1tr = Rẻ, 1tr–10tr = Trung bình, >10tr = Cao cấp).
select p.id as product_id, p.name as product_name, p.price as original_product,
case
	when p.price < 1000000 then "Re"
    when p.price > 1000000 and p.price < 10000000 then "Trung binh"
    when p.price > 10000000 then "Cao cap"
 else "Khong the xac dinh !" 
 end as price_classification_product
 from products p;

-- 3. COALESCE: Lấy danh sách addresses, nhưng nếu city bị NULL thì thay bằng "Chưa cập nhật".
select * from addresses;
select a.id as address_id, a.street as address_street, coalesce(a.city, "Chua cap nhat") as addess_city, a.country as address_country
from addresses a;

-- 4. NULLIF: Lấy users.email, nhưng nếu email = 'bao@ut.edu.vn' thì trả về NULL.
select * from users;
select u.id as user_id, u.name as user_name, nullif(u.email, "bao@ut.edu.vn") as user_email
from users u;

-- 5. CASE: Trong payments, hiển thị "Thanh toán thành công" nếu amount > 0, ngược lại "Chưa thanh toán".
select * from payments;
select p.id as payment_id, p.amount as payment_amount,
case
when p.amount > 0 then "Thanh toan thanh cong"
else "Chua thanh toan" 
end as status_payment
from payments p;


-- 6. CASE: Trong order_items, tính tổng doanh thu nhưng gắn nhãn "Cao" nếu price > 20tr, "Thấp" nếu ngược lại.
select * from order_items;
select * from orders;
select oi.id as order_item_id, oi.price as order_item_price,
case
when oi.price > 20000000 then "Cao"
when oi.price < 20000000 then "Thap"
else "Khong xac dinh" 
end as order_item_in_price
from order_items oi;

-- 7. CASE + JOIN: Lấy danh sách orders kèm tên users, và hiển thị "Khách VIP" nếu user đã có > 2 đơn hàng.
select * from users;
select * from orders;
select o.id as order_id, o.order_date as order_date, o.status as order_status, u.name as user_name, count(o.id) as total_order,
case 
	when count(o.id) > 2 then "Khach VIP"
    when count(o.id) < 2 then "Khach binh thuong"
    else "Khong xac dinh"
end as classify_user
from orders o 
join users u on u.id=o.user_id
group by o.id, u.name, o.status, o.order_date
order by o.id;


-- 8. COALESCE: Khi join suppliers và products, nếu contact_email bị NULL thì thay bằng "no-email@supplier.com".
select * from suppliers;
select * from products;


NULLIF: Trong reviews, nếu rating = 1 thì hiển thị NULL, còn lại giữ nguyên.

CASE: Hiển thị top sản phẩm theo reviews.rating: "Tốt (4–5)", "Trung bình (2–3)", "Kém (1)".

Phần 3: Khó hơn (thực tế & báo cáo)

CASE: Tạo báo cáo orders → "Hoàn tất nhanh" nếu Delivered trong ≤ 3 ngày, "Giao lâu" nếu > 3 ngày.

CASE + JOIN: Hiển thị shipping.shipping_status nhưng dịch ra tiếng Việt (Pending = Đang chờ, In Transit = Đang giao, …).

COALESCE: Khi join users và addresses, nếu không có địa chỉ thì hiển thị "Chưa có địa chỉ".

CASE: Trong inventory, phân loại tình trạng kho: "Hết hàng" (stock=0), "Sắp hết" (<20), "Đủ" (>=20).

CASE + GROUP: Đếm số lượng đơn hàng mỗi user theo nhóm "Nhiều đơn" (>3), "Ít đơn" (<=3).

Phần 4: Ứng dụng Thực tế

CASE: Trong báo cáo doanh thu (order_items), gắn nhãn "Doanh thu cao" nếu tổng order > 30tr, "Thấp" nếu ngược lại.

CASE: Khi join payments, hiển thị "Trả sau" nếu method = "COD", "Online" cho các phương thức khác.

COALESCE: Lấy danh sách sản phẩm và tồn kho, nếu stock_quantity NULL thì mặc định = 0.

CASE: Trong reviews, phân loại cảm nhận khách hàng: "Tích cực" (rating>=4), "Trung lập" (rating=3), "Tiêu cực" (1–2).

CASE + JOIN: Tạo bảng báo cáo: user name, sản phẩm đã mua, và tình trạng "Đang xử lý", "Hoàn tất" dựa vào orders.status.
